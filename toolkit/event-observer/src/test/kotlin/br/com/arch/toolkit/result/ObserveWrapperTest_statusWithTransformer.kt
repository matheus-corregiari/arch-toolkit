...
class ObserveWrapperTest_statusWithTransformer {
    ...
    private val mockedStatus: (Int) -> Unit = mock()
    private val mockedTransformer: (DataResultStatus) -> Int = mock()
    ...
    @Before
    fun init() {
        whenever(mockedStatus.invoke(123)) doReturn Unit
    ...
    fun `001 - single=false - null, null, SUCCESS`() = test(
        ...
        verifyBlocking(mockedTransformer) { invoke(status) }
        verifyBlocking(mockedStatus) { invoke(123) }
