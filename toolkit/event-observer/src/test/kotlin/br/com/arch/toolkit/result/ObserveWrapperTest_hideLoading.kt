...
class ObserveWrapperTest_hideLoading {

    private val error = IllegalStateException("Error!")
    private val data = "data"
    private val mockedLoading: () -> Unit = mock()
    ...
    @Before
    fun init() {
        whenever(mockedLoading.invoke()) doReturn Unit
    }
    ...
    fun `001 - single=false - null, null, SUCCESS`() = test(
        ...
        verifyBlocking(mockedLoading) { invoke() }
    ...
    private inline fun <T> test(
        ...
        if (single) {
            wrapper.hideLoading(single = true, observer = mockedLoading)
        } else {
            wrapper.hideLoading(observer = mockedLoading)
        }
        verifyNoInteractions(mockedLoading)
        assertEquals(1, wrapper.eventList.size)
    ...
    private inline fun <T> test(
        ...
        val wrapper = ObserveWrapper<T>()
        if (single) {
            wrapper.hideLoading(single = true, withData = withData, observer = mockedLoading)
        } else {
            wrapper.hideLoading(withData = withData, observer = mockedLoading)
        }
        verifyNoInteractions(mockedLoading)
        assertEquals(1, wrapper.eventList.size)
