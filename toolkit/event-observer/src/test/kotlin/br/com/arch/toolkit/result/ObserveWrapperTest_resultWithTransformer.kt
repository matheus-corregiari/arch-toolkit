...
class ObserveWrapperTest_resultWithTransformer {
    ...
    private val mockedResult: (Int) -> Unit = mock()
    private val mockedListResult: (Int) -> Unit = mock()
    private val mockedTransformer: (DataResult<String>) -> Int = mock()
    private val mockedListTransformer: (DataResult<List<String>>) -> Int = mock()
    ...
    @Before
    fun init() {
        whenever(mockedResult.invoke(123)) doReturn Unit
        whenever(mockedListResult.invoke(123)) doReturn Unit
        whenever(mockedTransformer.invoke(any())) doReturn 123
        whenever(mockedListTransformer.invoke(any())) doReturn 123
    }
    ...
    fun `001 - single=false - null, null, SUCCESS`() = test(
        ...
        verifyBlocking(mockedResult) { invoke(123) }
    ...
    fun `017 - single=false - empty, null, SUCCESS`() = testList(
        ...
        verifyBlocking(mockedListTransformer) { invoke(result) }
        verifyBlocking(mockedListResult) { invoke(123) }
        assertEquals(1, wrapper.eventList.size)
    }
