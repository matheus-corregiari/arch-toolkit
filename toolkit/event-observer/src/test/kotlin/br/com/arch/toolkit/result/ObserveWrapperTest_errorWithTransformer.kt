...
class ObserveWrapperTest_errorWithTransformer {
    ...
    private val mockedTransformer: (Throwable) -> String = mock()
    private val mockedError: (String) -> Unit = mock()
    ...
    @Before
    fun init() {
        whenever(mockedTransformer.invoke(error)) doReturn data
        whenever(mockedError.invoke(data)) doReturn Unit
    }
    ...
    fun `011 - single=false - null, error, ERROR`() = test(
        ...
        verifyBlocking(mockedError) { invoke(data) }
        assertEquals(1, wrapper.eventList.size)
    }
