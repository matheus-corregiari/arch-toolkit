...
class LumberInfoTest {
    ...
    fun `info with very long message exceeding MAX LOG LENGTH`() {
        val tree = TestTree()
        Lumber.plant(tree)
        Lumber.info("a".repeat(MAX_LOG_LENGTH + 2000))
        assertEquals(listOf(tree), Lumber.forest())
        tree.assertAll(
            TestTree.Data(
                level = Lumber.Level.Info,
    ...
    fun `info with message exactly MAX LOG LENGTH`() {
        val tree = TestTree()
        Lumber.plant(tree)
        Lumber.info("a".repeat(MAX_LOG_LENGTH))
        assertEquals(listOf(tree), Lumber.forest())
        tree.assertAll(
            TestTree.Data(
                level = Lumber.Level.Info,
    ...
    fun `info with explicit tag and long message`() {
        val tree = TestTree()
        Lumber.plant(tree)
        Lumber.tag("MyTag").info("a".repeat(MAX_LOG_LENGTH + 2000))
        assertEquals(listOf(tree), Lumber.forest())
        tree.assertAll(
            TestTree.Data(
                level = Lumber.Level.Info,
    ...
    fun `info when isLoggable is false with exception`() {
        ...
        Lumber.info(Exception("nope"), "normal text without arguments")
    ...
    fun `info when quiet is true with exception`() {
        ...
        Lumber.info(Exception("quiet"), "normal text without arguments")
