...
class ObserveWrapperTest_errorWithTransformer {

    private val error = IllegalStateException("Error!")
    private val data = "data"
    private val mockedTransformer: (Throwable) -> String = mockk()
    private val mockedError: (String) -> Unit = mockk()
    ...
    @BeforeTest
    fun init() {
        every { mockedTransformer.invoke(error) } returns data
        every { mockedError.invoke(data) } returns Unit
    }
    ...
    fun `011 - single=false - null - error - ERROR`() = test(
        ...
        verify(exactly = 1) { mockedTransformer.invoke(error) }
        verify(exactly = 1) { mockedError.invoke(data) }
        assertEquals(1, wrapper.eventList.size)
    }
