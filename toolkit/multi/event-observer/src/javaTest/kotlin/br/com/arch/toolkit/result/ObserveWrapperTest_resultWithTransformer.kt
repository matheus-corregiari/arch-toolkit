...
class ObserveWrapperTest_resultWithTransformer {

    private val error = IllegalStateException("Error!")
    private val data = "data"
    private val mockedResult: (Int) -> Unit = mockk()
    private val mockedListResult: (Int) -> Unit = mockk()
    private val mockedTransformer: (DataResult<String>) -> Int = mockk()
    private val mockedListTransformer: (DataResult<List<String>>) -> Int = mockk()
    ...
    @BeforeTest
    fun init() {
        every { mockedResult.invoke(123) } returns Unit
        every { mockedListResult.invoke(123) } returns Unit
        every { mockedTransformer.invoke(any()) } returns 123
        every { mockedListTransformer.invoke(any()) } returns 123
    }
    ...
    fun `001 - single=false - null - null - SUCCESS`() = test(
        ...
        verify(exactly = 1) { mockedTransformer.invoke(result) }
        verify(exactly = 1) { mockedResult.invoke(123) }
    ...
    fun `013 - single=false - null - null - NONE`() = test(
        ...
        verify(exactly = 0) { mockedResult.invoke(any()) }
        assertEquals(1, wrapper.eventList.size)
    }
    ...
    fun `017 - single=false - empty - null - SUCCESS`() = testList(
        ...
        verify(exactly = 1) { mockedListTransformer.invoke(result) }
        verify(exactly = 1) { mockedListResult.invoke(123) }
        assertEquals(1, wrapper.eventList.size)
    }
    ...
    private inline fun testList(
        ...
        if (single) {
            wrapper.result(
                single = true,
                transformer = mockedListTransformer,
                observer = mockedListResult
            )
        } else {
            wrapper.result(
                transformer = mockedListTransformer,
        ...
        verify(exactly = 0) { mockedListTransformer.invoke(any()) }
        verify(exactly = 0) { mockedListResult.invoke(any()) }
        assertEquals(1, wrapper.eventList.size)
