...
class ObserveWrapperTest_errorEmpty {

    private val error = IllegalStateException("Error!")
    private val data = "data"
    private val mockedError: () -> Unit = mockk()
    ...
    @BeforeTest
    fun init() {
        every { mockedError.invoke() } returns Unit
    }
    ...
    fun `001 - single=false - null - null - SUCCESS`() = test(
        ...
        verify(exactly = 0) { mockedError.invoke() }
    ...
    fun `009 - single=false - null - null - ERROR`() = test(
        ...
        verify(exactly = 1) { mockedError.invoke() }
        assertEquals(1, wrapper.eventList.size)
    }
