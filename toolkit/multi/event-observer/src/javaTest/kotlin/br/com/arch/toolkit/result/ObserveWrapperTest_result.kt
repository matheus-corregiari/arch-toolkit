...
class ObserveWrapperTest_result {
    ...
    private val mockedResult: (DataResult<String>) -> Unit = mockk()
    private val mockedListResult: (DataResult<List<String>>) -> Unit = mockk()
    ...
    @BeforeTest
    fun init() {
        every { mockedResult.invoke(any()) } returns Unit
        every { mockedListResult.invoke(any()) } returns Unit
    }
    ...
    fun `001 - single=false - null - null - SUCCESS`() = test(
        ...
        verify(exactly = 1) { mockedResult.invoke(result) }
    ...
    fun `017 - single=false - empty - null - SUCCESS`() = testList(
        ...
        verify(exactly = 1) { mockedListResult.invoke(result) }
        assertEquals(1, wrapper.eventList.size)
    }
