...
class ObserveWrapperTest_success {

    private val error = IllegalStateException("Error!")
    private val data = "data"
    private val mockedSuccess: () -> Unit = mockk()
    ...
    @BeforeTest
    fun init() {
        every { mockedSuccess.invoke() } returns Unit
    }
    ...
    fun `001 - single=false - null - null - SUCCESS`() = test(
        ...
        verify(exactly = 1) { mockedSuccess.invoke() }
    ...
    fun `001 - single=false - withData=true - null - null - SUCCESS`() = test(
        ...
        verify(exactly = 0) { mockedSuccess.invoke() }
        assertEquals(1, wrapper.eventList.size)
    }
    ...
    private inline fun <T> test(
        ...
        if (single) {
            wrapper.success(single = true, observer = mockedSuccess)
        } else {
            wrapper.success(observer = mockedSuccess)
        }
        verify(exactly = 0) { mockedSuccess.invoke() }
        assertEquals(1, wrapper.eventList.size)
