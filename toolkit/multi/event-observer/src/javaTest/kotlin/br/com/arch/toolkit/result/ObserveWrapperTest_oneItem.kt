...
class ObserveWrapperTest_oneItem {

    private val error = IllegalStateException("Error!")
    private val data = "data"
    private val mockedOneItem: () -> Unit = mockk()
    ...
    @BeforeTest
    fun init() {
        every { mockedOneItem.invoke() } returns Unit
    }
    ...
    fun `001 - single=false - null - null - SUCCESS`() {
        test(
            ...
            verify(exactly = 0) { mockedOneItem.invoke() }
    ...
    fun `018 - single=false - notEmpty - null - SUCCESS`() = test(
        ...
        verify(exactly = 1) { mockedOneItem.invoke() }
        assertEquals(1, wrapper.eventList.size)
    }
    ...
    private inline fun <T> test(
        ...
        if (single) {
            wrapper.oneItem(single = true, observer = mockedOneItem)
        } else {
            wrapper.oneItem(observer = mockedOneItem)
        }
        verify(exactly = 0) { mockedOneItem.invoke() }
        assertEquals(1, wrapper.eventList.size)
