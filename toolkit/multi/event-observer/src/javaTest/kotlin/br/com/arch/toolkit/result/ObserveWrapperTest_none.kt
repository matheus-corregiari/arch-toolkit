...
class ObserveWrapperTest_none {

    private val error = IllegalStateException("Error!")
    private val data = "data"
    private val mockedNone: () -> Unit = mockk()
    ...
    @BeforeTest
    fun init() {
        every { mockedNone.invoke() } returns Unit
    }
    ...
    fun `001 - single=false - null - null - SUCCESS`() = test(
        ...
        verify(exactly = 0) { mockedNone.invoke() }
    ...
    fun `013 - single=false - null - null - NONE`() = test(
        ...
        verify(exactly = 1) { mockedNone.invoke() }
        assertEquals(1, wrapper.eventList.size)
    }
    ...
    private inline fun <T> test(
        ...
        if (single) {
            wrapper.none(single = true, observer = mockedNone)
        } else {
            wrapper.none(observer = mockedNone)
        }
        verify(exactly = 0) { mockedNone.invoke() }
        assertEquals(1, wrapper.eventList.size)
