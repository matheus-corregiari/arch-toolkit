...
class ObserveWrapperTest_notEmpty {

    private val error = IllegalStateException("Error!")
    private val data = "data"
    private val mockedNotEmpty: () -> Unit = mockk()
    ...
    @BeforeTest
    fun init() {
        every { mockedNotEmpty.invoke() } returns Unit
    }
    ...
    fun `001 - single=false - null - null - SUCCESS`() {
        test(
            ...
            verify(exactly = 0) { mockedNotEmpty.invoke() }
    ...
    fun `018 - single=false - notEmpty - null - SUCCESS`() = test(
        ...
        verify(exactly = 1) { mockedNotEmpty.invoke() }
        assertEquals(1, wrapper.eventList.size)
    }
    ...
    private inline fun <T> test(
        ...
        if (single) {
            wrapper.notEmpty(single = true, observer = mockedNotEmpty)
        } else {
            wrapper.notEmpty(observer = mockedNotEmpty)
        }
        verify(exactly = 0) { mockedNotEmpty.invoke() }
        assertEquals(1, wrapper.eventList.size)
