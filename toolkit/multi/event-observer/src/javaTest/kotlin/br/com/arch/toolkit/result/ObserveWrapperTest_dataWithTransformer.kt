...
class ObserveWrapperTest_dataWithTransformer {
    ...
    private val mockedData: (Int) -> Unit = mockk()
    private val mockedTransformer: (String) -> Int = mockk()
    private val mockedListTransformer: (List<String>) -> Int = mockk()
    ...
    @BeforeTest
    fun init() {
        every { mockedData.invoke(1234) } returns Unit
        every { mockedData.invoke(123) } returns Unit
        every { mockedData.invoke(321) } returns Unit
        every { mockedTransformer.invoke(data) } returns 1234
        every { mockedListTransformer.invoke(emptyList()) } returns 123
        every { mockedListTransformer.invoke(listOf(data)) } returns 321
    }
    ...
    fun `002 - single=false - data - null - SUCCESS`() = test(
        ...
        verify(exactly = 1) { mockedData.invoke(1234) }
        assertEquals(1, wrapper.eventList.size)
    }
    ...
    fun `017 - single=false - empty - null - SUCCESS`() = testList(
        ...
        verify(exactly = 1) { mockedListTransformer.invoke(emptyList()) }
        verify(exactly = 1) { mockedData.invoke(123) }
        assertEquals(1, wrapper.eventList.size)
    }
    ...
    fun `018 - single=false - notEmpty - null - SUCCESS`() = testList(
        ...
        verify(exactly = 1) { mockedListTransformer.invoke(listOf(data)) }
        verify(exactly = 1) { mockedData.invoke(321) }
        assertEquals(1, wrapper.eventList.size)
    }
