...
class ObserveWrapperTest_manyItems {
    ...
    private val mockedManyItems: () -> Unit = mock()
    ...
    @Before
    fun init() {
        whenever(mockedManyItems.invoke()) doReturn Unit
    }
    ...
    fun `001 - single=false - null, null, SUCCESS`() {
        test(
            ...
            verifyNoInteractions(mockedManyItems)
    ...
    fun `019 - single=false - manyItems, null, SUCCESS`() = test(
        ...
        verifyBlocking(mockedManyItems) { invoke() }
        assertEquals(1, wrapper.eventList.size)
    }
